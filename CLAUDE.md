...
- `.\scripts\capture_scene.ps1` captures the whole-store scenes into `docs\verification\ui_<scene>.png`. **Use it instead of the shared `..\macroquad-toolkit\scripts\capture_ui.ps1`** — that one runs the debug exe, which at ~4500 toys takes minutes per capture and in practice never finishes; this runs the release build and takes seconds. Scenes are staged in `src/capture_scenes.rs`, which sweeps the floor around the fixture under test so it is not buried in loose toys.
