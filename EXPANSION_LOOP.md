...
- [x] **Displays at scale**: many more display definitions across zones in `displays.json` (multiple displays per category, per zone). Capacity math stays exact against `toy_count`.
- [x] **Perf gate B: `toy_count` → 1500.** 60 FPS native confirmed; scatter logic distributes toys across all zones (deterministic clusters/piles, not uniform noise). Log FPS.
- [x] **Perf gate C: `toy_count` → 4000+.** 60 FPS native, WebGL still playable (log its FPS too). This is the headline target — if it fails, add and complete the needed optimization tasks (e.g. cheaper LOD primitive, draw batching, tighter culling) before raising the count.
- [x] **Phase gate:** `.\publish.ps1`, verify the 4000-toy store in browser, update README player-facing description, commit.
...
- 2026-07-12 — Displays at scale — displays.json now 20 displays (4 per category × 25 capacity = 500), placed inside their theme zones with walkable gaps. `DisplayStyle` enum + `display_style()` (substring on id: pegboard/wall/shelf/table/bin/generic) replaces exact-id dispatch in fixtures.rs, placed_height_for_slot, and signs.rs sign_layout; stock-sign titles now come from category. One theme per category kept (any plushie fits any plushie display). GOTCHA fixed: capacity-25 made the spawn-pose selector all-even ((4d+2s)%8) locking out upside-down poses — slot coefficient 3→4; test helper now skips already-placed toys (same-theme sibling displays share matches). Bench 500 toys: 201.6 FPS (20 fixtures cost ~50 FPS vs 5 — watch at gate C; per-display culling is the lever if needed). Gate B (1500) next: raise capacities to 75 each.
- 2026-07-12 — Perf gate B PASSED — toy_count=1500 (20 displays × 75), 15s bench: 71.8 avg FPS native release. Scatter stays deterministic weighted piles across all zones (13 anchors). Margin over 60 is thin: extrapolating ~linear cost, 4000 will NOT pass as-is — gate C should start by tightening `toy_render_distance` (22 → ~14) and `toy_lod_distance` (9 → ~6), then re-bench before touching code; placed-toy per-display culling is the next lever.
- 2026-07-12 — Perf gate C PASSED (headline!) — toy_count=4000 (20 displays × 200). Distances tuned in game_config.json: `toy_render_distance` 22→14, `toy_lod_distance`/`toy_pose_distance` 9→6. Bench (native release, 20s): baseline 26.2 FPS at old distances → **282.3 avg FPS** at 14/6. NOTE: cost is violently nonlinear in render distance (17/7.5 gave only 35 FPS — likely a render-batch buffer threshold); treat 14/6 as a cliff-edge-safe setting, don't casually raise. Bug fixed en route: `keep_off_fixtures` nudges toward a wall got clamped back inside edge-adjacent fixtures — nudge targets now restricted to the playable band + two passes for chain nudges (caught by floor-spawn test at 4000 density). WebGL FPS not headless-measurable — measure in browser at the Phase 2 gate (publish next iteration).
- 2026-07-12 — Phase 2 gate — publish.ps1 clean (4000-toy wasm deployed, all URLs 200, wasm magic valid, Project Roost notified); README "Current Prototype" section rewritten as "The Store" describing the full 4000-toy/7-zone/20-display/50-design game. WebGL in-browser FPS still needs human eyes at `http://127.0.0.1/games/toybox/` (F3 for the overlay) — flagged to user each publish. Phase 2 complete — Phase 3 (broken toys at scale) next.
