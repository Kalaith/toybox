...
- **Repairs are close to unreachable as tuned.** A replay closer working nearest-first completes *zero* repairs in 400 actions and defers 58-103 parts: heads scatter into a different zone from their body, so the two halves are never both to hand. Either shorten the scatter, let a bench hold parts from more than one break, or give the scanner a "fetch the pair" route. Until then ~12% of the shop cannot be shelved at all.
- A bench holding two mismatched halves is a dead end — every later part is refused and there is no prompt telling the player to clear it. The replay had to grow a deferral list to avoid an infinite pick-up/refuse/drop loop at the bench.
